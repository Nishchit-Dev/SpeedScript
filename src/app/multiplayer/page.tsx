...
export default function Typing() {
  ...
  useEffect(() => {
    ...
    const onlyAlphabetsAndSpace = !buttons.withSymbols
      ? [...characterArray].filter((char) => /[a-zA-Z\u2000\s]/.test(char))
      : characterArray;
  ...
  return (
    <>
      <div className="p-2 bg-blue-400 text-white absolute top-0 mt-4 ml-4 rounded-md">
        <span className=" text-gray-100 p-2">room ID :</span> {roomData.roomID}
      </div>
      {!style ? (
        <div
          className={clsx(
            {
              "relative w-full justify-center items-center": !gameOver,
            },
            {
              invisible: gameOver,
            }
        ...
        >
          ...
          ></div>
          {Object.entries(roomData.roomInfo).map(([player, state], index) => {
            ...
            return (
              <div
                ...
                style={{
                  transform: `translate(${
                    Number(getPosition(typedState.currentPosition)?.x ?? 0) - 2
                  }px, ${
                    Number(getPosition(typedState.currentPosition)?.y ?? 0) - 99
                  }px)`,
                }}
              ></div>
            );
          ...
          <div
            ...
            style={{
              transform: `translate(${ghostCursorPosition.x - 2}px, ${
                ghostCursorPosition.y - 99
              }px)`,
            }}
          ></div>
        </div>
                      ...
                      <div className="p-2 bg-white" key={player}>
                        <div
                          className={clsx(
                            ...
                            {
                              "opacity-100": typedValues.isReady,
                            },
                            {
                              "opacity-50": !typedValues.isReady,
                            }
                          )}
                        >
                          <p>{player}</p>
          ...
          >
            {characterArray.length > 0 ? (
              characterArray.map((character, index) => {
                if (
                  index >= numberOfCharacters * (multiplier - 1) &&
                  ...
                  return (
                    <p
                      ...
                      className={clsx(
                        `character${index}  `,
                        {
                          "text-gray-400": index > charIndex,
                        },
                        {
                          "text-black":
                            index < charIndex || !incorrectChar.includes(index),
                        },
                        {
                          "text-red-500":
                            index < charIndex &&
...

                        {
                          ...
                          "text-green-500 cursorIsHere": index == charIndex,
                        }
                      )}
                    >
                      {character}
